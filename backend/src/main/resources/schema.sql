...
-- ============================================
-- 3. 咨询表 (inquiries) - 已废弃，使用 conversations + messages 表
-- ============================================
...
--     status ENUM('pending', 'replied', 'closed') NOT NULL DEFAULT 'pending' COMMENT '咨询状态',
...
-- ============================================
-- 9. 对话表 (conversations)
-- 用途: 存储房东与租客之间的对话，每个对话关联一个房源
-- ============================================
CREATE TABLE IF NOT EXISTS conversations (
    id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '对话主键ID',
    property_id BIGINT UNSIGNED NOT NULL COMMENT '房源ID，外键关联properties.id',
    landlord_id BIGINT UNSIGNED NOT NULL COMMENT '房东ID，外键关联users.id',
    tenant_id BIGINT UNSIGNED NOT NULL COMMENT '租客ID，外键关联users.id',
    status ENUM('active', 'closed') NOT NULL DEFAULT 'active' COMMENT '对话状态: active(进行中) / closed(已关闭)',
    last_message VARCHAR(500) DEFAULT NULL COMMENT '最后一条消息预览（最多500字符）',
    last_message_at TIMESTAMP NULL COMMENT '最后消息时间',
    unread_tenant_count INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '租客未读消息数',
    unread_landlord_count INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '房东未读消息数',
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (id),
    UNIQUE KEY uk_property_tenant (property_id, tenant_id),
    KEY idx_landlord_id (landlord_id),
    KEY idx_tenant_id (tenant_id),
    KEY idx_last_message_at (last_message_at),
    CONSTRAINT fk_conversations_property FOREIGN KEY (property_id)
        REFERENCES properties(id) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT fk_conversations_landlord FOREIGN KEY (landlord_id)
        REFERENCES users(id) ON DELETE CASCADE ON UPDATE CASCADE,
...
CREATE TABLE IF NOT EXISTS messages (
    ...
    conversation_id BIGINT UNSIGNED NOT NULL COMMENT '对话ID，外键关联conversations.id',
